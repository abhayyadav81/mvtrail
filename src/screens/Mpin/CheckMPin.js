...
const MPINScreen = ({ navigation }) => {
  ...
  const fetchApi = async () => {
    ...
    try {
      ...
      const response = await axios.post(`${BASE_URL}verifyMPIN`, formData, {
        headers: {
      ...
      if (response.data?.text === "MPIN verification successful") {
  ...
  return (
    <View style={styles.container}>
      ...
      <TouchableOpacity onPress={()=>navigation.navigate(RouteName.LOGIN_SCREEN,{data:1})}>
        <Text style={{ color: "white", fontSize: 17 }}>Forgot Pincode ? Login</Text>
      </TouchableOpacity>
    </View>
  );
};
